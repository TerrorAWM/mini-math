...
int main() {
    ...
    while (1) {
        ...
        printf("Already logged in, Welcome back!\n");
        ...
        while (stay_in_menu) {
            printf("\n====== Game Panel ======\n");
            printf("1. Number Comparing Game\n");
            // printf("2. 人机比大小 (与本地机器人游玩)\n");
            printf("LG. Log Out\n");
...
int login() {
    ...
    printf("If not registered please visit the following link to register:\n");
    printf("%s\n\n", REGISTER_URL);
...
void start_comparison_game() {
    ...
    if (connect(sock, (struct sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
        printf("Server connection failed, please retry or select another mode.\n");
        close(sock);
        return;
    }
    ...
    while (1) {
        ...
        if (strcmp(choice, "1") == 0) {
            ...
            if (bytes_received <= 0) {
                printf("Failed to receive room code.\n");
                close(sock);
                return;
            }
            buffer[bytes_received] = '\0';
            printf("Room created success, Room code: %s\n", buffer);
            printf("Share code to friends\n");
            break;
        } else if (strcmp(choice, "3") == 0) {
            // 加入房间
            send(sock, "JOIN\n", strlen("JOIN\n"), 0);
            printf("Please enter a Room code: ");
            ...
            if (strcmp(buffer, "Joined room\n") != 0) {
                printf("Unable to join the room: %s\n", buffer);
                close(sock);
                return;
            }
            printf("Join the room success.\n");
            break;
        } else {
